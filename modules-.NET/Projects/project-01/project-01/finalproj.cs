    ...
    class Program
    {
        ...
        public static void SaveToHistory(int CurrWorkingUserID, string ParticipantUsers, int WhoPaidTheBillID, int Moneyamount, DateTime dateTime)
        {
            ...
            for (int i = 0; i < participantsIds.Count; i++)
            {
                ...
                historyMembersLine += $" {listOfName[mm-1]},";
            }
            foreach (var item in listOfName)
            {
                newListOfName += $" {item},";
            }
            int xx = CurrWorkingUserID;
            int yy = WhoPaidTheBillID;

            finalString = $"{finalString}\n " +
                $"--------------------------------------------------\n" +
                $"{listOfName[CurrWorkingUserID-1]} added this event and ToTal cost is: {Moneyamount}\n" +
                $"Date: {dateTime}\n" +
                $"Members:{historyMembersLine}\n" +
                $"Paid by:{listOfName[WhoPaidTheBillID-1]}\n";
